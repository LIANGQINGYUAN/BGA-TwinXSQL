...
def compute_accuracy(reference_strings, predicted_strings):
    ...
    for i in range(len(reference_strings)):
        # if ' '.join(tokenize_for_bleu_eval(reference_strings[i])) == ' '.join(tokenize_for_bleu_eval(predicted_strings[i])):
...
def compute_gleu(len_data, orig_file, ref_file, pred_file):
    ...
    command = '/home/lqy/anaconda3/envs/py27/bin/python  {} -s {} -r {} -o {} -d'.format(
