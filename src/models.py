...
def enrich_vocab(
    ...
    if 'MSPv2' in args.load or 'MIPv2' in args.load or 'CodeT5v2' in args.load or 'base' in args.load or 'mlp' in args.load:
        add_tokens += ["<mtp>", "<mvp>", '<mip>', '<msp>']
    if 'link' in args.load:
        print('pretraining with link....')
        add_tokens += ["<mtp>", "<mvp>", "<msp>", "<mip>", "<mlp>"]
    else:
        add_tokens += ["<mtp>", "<mvp>"]
