...
def eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, split_tag, criteria, accelerator):
    ...
    # pred_nls = [' '.join(tokenize_for_bleu_eval(c)) for c in pred_nls]
    # golds = [' '.join(tokenize_for_bleu_eval(c.target)) for c in eval_examples]
    pred_nls = [c for c in pred_nls]
    golds = [c.target for c in eval_examples]
    ...
    with open(output_fn, 'w') as f, open(gold_fn, 'w') as f1, open(src_fn, 'w') as f2:
        for pred_nl, ex, gold in zip(pred_nls, eval_examples, golds):
            dev_accs.append(pred_nl.strip() == gold.strip())
            f.write(json.dumps({"pred":pred_nl.strip()})+ '\n')
            f1.write(json.dumps({"gold":gold}) + '\n')
            f2.write(json.dumps({"source":ex.source.strip()}) + '\n')
    if ex.target.strip():
        # bleu = round(_bleu(gold_fn, output_fn), 2)
        bleu = round(bleu_from_list([g.strip() for g in golds], [p.strip() for p in pred_nls]),2)
        # if 'J2C' in args.task:
...
def main(args):
    ...
    if 'outputs' in args.load.lower():
        ...
        # model = accelerator.unwrap_model(model)
        model.load_state_dict(torch.load(loading_path, map_location="cpu")['model'])
        logger.info("Load model from {}".format(loading_path))

    # codereviewer: microsoft/codereviewer
    elif 'codereviewer' in args.load or 'CoditT5' in args.load or 'plbart' in args.load:
